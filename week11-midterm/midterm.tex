...
\begin{homeworkProblem}[Problem 8]
    ...
    \textbf{C.} The expected reward assuming that the actions are taken by the behavior policy
    is given by,
    ...
    \begin{align*}
        Var[X] &= \EX[X^2] - \EX[X]^2\\
        &= (10^2 \cdot 0.5 + 20^2 \cdot 0.5) - 15^2\\
        &= 75
    \end{align*}
    ...
    Importance sampling increases the variance in off-policy learning because it is a ratio
    of the target policy and the behavior policy. Therefore, if the behavior policy is very
    different from the target policy, then the importance sampling ratio will be very large,
    which will increase the variance of the expected reward. This is because the importance
    sampling ratio is multiplied with the reward, and the reward is squared in the variance
    calculation.\\

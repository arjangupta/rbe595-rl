...
\begin{homeworkProblem}[Problem 3]
    ...
    We know the equation for $q(s, a)$ is given by,

    \begin{align*}
        q(s, a) &= \sum_{s', r} p(s', r | s, a) \left[ r + \gamma V(s') \right]
    \end{align*}

    \textbf{A.} We can calculate $q(k, down)$ as follows:

    \begin{align*}
        q(k, down) &= \sum_{s', r} p(s', r | k, down) \left[ r + \gamma V(s') \right] \\
        &= p(k, -1 | k, down) \left[ -1 + \gamma V(k) \right] + p(f, -1 | k, down) \left[ -1 + \gamma V(f) \right] \\
        &= 1 \left[ -1 + \gamma V(k) \right] + 0 \left[ -1 + \gamma V(f) \right] \\
        &= -1 + \gamma V(k) \\
        &= -1 + 0.5 \\
        &= -0.5
    \end{align*}
